...
ROOT = FILE.parents[1]  # YOLO_xyz
ASSETS = ROOT / "assets"  # default images
WEIGHTS = ROOT / "weights"  # default model weights
DEFAULT_CFG_PATH = ROOT / "cfg/default.yaml"
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of YOLO_xyz multiprocessing threads
...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        # Load a model
        model = YOLO_xyz("yolov8n.yaml")  # build a new model from scratch
        model = YOLO_xyz("yolov8n.pt")  # load a pretrained model (recommended for training)
...
def set_sentry():
    ...
    Conditions required to send errors (ALL conditions must be met or no errors will be reported):
        ...
        - sync=True in YOLO_xyz settings
...
class SettingsManager(dict):
    ...
    def __init__(self, file=SETTINGS_YAML, version="0.0.5"):
        ...
        # print(datasets_root, root)
        self.file = Path(file)
        self.version = version
        self.defaults = {
            "settings_version": version,
            "datasets_dir": str(datasets_root / "exp" / "datasets"),
            "weights_dir": str(datasets_root / "exp" / "weights"),
            "runs_dir": str(datasets_root / "exp" / "runs"),
